...
Begin by opening VSCode and utilizing ampy to retrieve the files from the ESP8266.

```bash
ampy -p (COM-PORT) get boot.py
``` 
Next, create a file named boot.py and paste all the content retrieved from the previous command into this file.

#### Step 2:
Modify the file to transmit a different variable to the leaderboard server. Below is the edited boot.py file (the complete version is located at the bottom of the page):

```python
try:
    m = umqtt.simple.MQTTClient(client_id, config['score_server'], user=config['team_num'], password=config['team_key'])
    m.connect(True)
    m.publish(b"score/%s" % config['team_num'], b'{"score":%d}' % 10000)
    m.disconnect()
    submit_success = True
```
...
def display_home():
    ...
    chef = make_image("flag", 100, 40, False)
